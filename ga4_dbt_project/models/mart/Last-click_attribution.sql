...
select distinct
    user_pseudo_id,
    first_value(channel) over (partition by user_pseudo_id order by session_end desc) as last_channel,
    max(session_end) over (partition by user_pseudo_id) as last_touch_time,
